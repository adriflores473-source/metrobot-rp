...
client.on('interactionCreate', async interaction => {
    ...
    const ROL_AUTORIZADO_ACTIVITY = '1510142095262875768';
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'agregar-busqueda') {
            ...
            try {
                await usuario.roles.add(ROL_BUSQUEDA_ID);
                await interaction.reply({ content: `🚨 **${usuario.user.username}** ha sido puesto en búsqueda.` });
            } catch (e) { await interaction.reply({ content: '❌ Error: Revisa mi jerarquía de roles.', ephemeral: true }); }
        }
        ...
        if (interaction.commandName === 'retirar-busqueda') {
            ...
            const usuario = interaction.options.getMember('usuario');
            try {
                await usuario.roles.remove(ROL_BUSQUEDA_ID);
                await interaction.reply({ content: `✅ Se ha retirado la búsqueda a ${usuario.user.username}.` });
            } catch (e) { await interaction.reply({ content: '❌ Error: Revisa mi jerarquía de roles.', ephemeral: true }); }
        }
    }
